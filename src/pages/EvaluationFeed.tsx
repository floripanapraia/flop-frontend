...
const EvaluationFeed: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"avaliacoes" | "fotos" | "flops">("avaliacoes");
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    setErrorAvaliacoes(null);
    if (praiaId) fetchEvaluationPage(1);
  }, [praiaId]);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      // Quando faltar menos de 150px para o fim, chama próxima página
      ...
      <div className="h-full flex flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col h-[calc(100vh-3rem)] lg:h-screen">
          ...
          <div className="bg-white border-b border-gray-200 py-2 flex justify-around shadow-sm sticky top-0 z-40">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-3 py-1 text-xs font-medium ${activeTab === tab.id ? "text-blue-600" : "text-gray-500 hover:text-gray-700"
                  }`}
              >
                <div className="flex flex-col items-center">
                  <span>{tab.label}</span>
                  {activeTab === tab.id && <div className="w-5 h-0.5 bg-blue-600 mt-1 rounded-full"></div>}
                </div>
              </button>
            ))}
          </div>
          ...
          <div
            ...
            className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50 space-y-4"
          >
            {/* Post */}
            {loadingAvaliacoes && <p className="text-gray-600">Carregando avaliações de hoje…</p>}
            {errorAvaliacoes && <p className="text-red-600">{errorAvaliacoes}</p>}
            {!loadingAvaliacoes && avaliacoes.length === 0 && <p className="text-gray-600">Nenhuma avaliação registrada hoje.</p>}
            ...
            {!loadingAvaliacoes &&
              avaliacoes.map((item) => {
                ...
                return (
                  <div key={item.idAvaliacao} className="bg-white rounded-lg p-4 shadow">
